...
module {
  ...
  // hw.module @mbx_fsm(in %clk : !seq.clock, out mbx_empty_o : i1, out mbx_write_o : i1, out mbx_read_o : i1, out mbx_sys_abort_o : i1, out mbx_ready_update_o : i1, out mbx_ready_o : i1, out mbx_irq_ready_o : i1, out mbx_irq_abort_o : i1, out mbx_state_error_o : i1) {
    // %rst_ni = verif.symbolic_value : i1
    // %mbx_range_valid_i  = verif.symbolic_value: i1
    // %hostif_abort_ack_i  = verif.symbolic_value: i1
    // %hostif_abort_ack_i  = hw.constant true
    // %mbx_error_set_i  = verif.symbolic_value: i1
    // %sysif_control_abort_set_i  = verif.symbolic_value: i1
    // %sys_read_all_i  = verif.symbolic_value: i1
    // %writer_close_mbx_i  = verif.symbolic_value: i1
    // %writer_last_word_written_i  = verif.symbolic_value: i1
    // %writer_write_valid_i  = verif.symbolic_value : i1
    ...
    %aff_ctrl_state_q.q_o = hw.constant 0 : i3
    // hw.instance "aff_ctrl_state_q" @prim_flop(clk: %clk: !seq.clock, rst_ni: %rst_ni: i1, d_i: %83: i3) -> (q_o: i3) {sv.namehint = "ctrl_state_logic"}
    ...
    %eqs = comb.icmp eq %83, %c0_i3 : i3
    verif.assume %eqs : i1
    // verif.assume_equal %83, %c3_i3 : i3
    // verif.assert_equal %83, %c3_i3 : i3
    // verif.ensure(comb.icmp)
